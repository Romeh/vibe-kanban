...
pub async fn pair_relay_host(
    ...
    if !cfg!(debug_assertions) {
        if let Ok(client) = deployment.remote_client() {
            let hosts = client.list_relay_hosts().await?;
            let selected_host = hosts
                .into_iter()
                .find(|host| host.id == req.host_id)
                .ok_or_else(|| {
                    ApiError::BadRequest("Selected host is not available".to_string())
                })?;
        ...
        // Local mode: skip remote host validation, proceed with local pairing
    }
