...
pub async fn link_workspace(
    ...
    match deployment.remote_client() {
        ...
        Err(_) => {
            // Local mode: store the link in SQLite
            let pool = &deployment.db().pool;
            Workspace::link_to_issue(pool, workspace.id, payload.project_id, payload.issue_id)
                .await?;
        }
    }
...
pub async fn unlink_workspace(
    ...
    match deployment.remote_client() {
        ...
        Err(_) => {
            // Local mode: clear the link in SQLite
            let pool = &deployment.db().pool;
            Workspace::unlink_from_issue(pool, workspace_id).await?;
            Ok(ResponseJson(ApiResponse::success(())))
        }
    }
}
